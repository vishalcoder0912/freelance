...
const quickActions = [
  ...
  { title: 'Counting', icon: '🔢', path: '/lkg/counting', color: 'orange' },
  { title: 'Writing', icon: '✍️', path: '/lkg/handwriting', color: 'purple' },
  { title: 'Math', icon: '➕', path: '/lkg/math-fun', color: 'red' },
  { title: 'Science', icon: '🔬', path: '/lkg/science-fun', color: 'teal' },
  { title: 'Games', icon: '🎮', path: '/lkg/interactive-games', color: 'pink' },
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-6 pb-24">
        ...
        <div className="mb-6">
          <ProgressBar
            ...
            color="bg-gradient-to-r from-kid-green via-kid-teal to-kid-blue"
            showPercent
            size="lg"
          />
        </div>
        ...
        <motion.div className="mb-8" variants={containerVariants} initial="hidden" animate="visible">
          <div className="flex items-center justify-between mb-4">
            ...
            <motion.button
              onClick={() => navigate('/lkg/interactive-games')}
              className="text-sm text-kid-green font-semibold flex items-center gap-1 hover:gap-2 transition-all"
              whileHover={{ x: 3 }}
            >
            ...
            {lkgModules.map((mod) => {
              ...
              return (
                ...
                >
                  ...
                  <ProgressBar value={completed} max={total} size="sm" color="bg-gradient-to-r from-kid-green to-kid-blue" />
                </motion.div>
              )
            })}
          </div>
        ...
        >
          <div className="flex justify-center gap-4 mb-4">
            <AnimatedCharacter name="Tina" emoji="🐱" size="lg" />
            <AnimatedCharacter name="Max" emoji="🐶" size="lg" />
            <AnimatedCharacter name="Ella" emoji="🐰" size="lg" />
          </div>
          <h2 className="text-2xl font-bold text-white font-fredoka mb-2">Ready to learn?</h2>
          <p className="text-white/80 mb-4">Pick a module and start your adventure!</p>
          <motion.button
            onClick={() => navigate('/lkg/phonics')}
            className="bg-white text-kid-green px-8 py-3 rounded-full font-bold text-lg shadow-lg inline-flex items-center gap-2"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          >
